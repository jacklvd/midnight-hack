...
import { analyzeImage, commitVerdict } from '@/lib/api'
    ...
    <div className="min-h-screen bg-background text-foreground">
      <main className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-secondary-foreground">TruthLens</p>
            <h1 className="mt-3 text-4xl font-semibold tracking-tight sm:text-5xl">Privacy-first deepfake verification</h1>
            <p className="mt-4 max-w-2xl text-lg leading-8 text-muted-foreground">
              Upload an image, analyze it with the backend AI engine, then commit a proof-backed verdict to Midnight.
            </p>
          </div>
          <div className="flex gap-3">
            <Link href="/verify" className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium transition hover:bg-muted">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
            ...
            <CardContent className="space-y-6">
              <div className="grid gap-4">
                ...
                >
                  ...
                  <span className="mt-2 text-sm text-muted-foreground">PNG, JPG, or WEBP; max 10 MB</span>
                  {preview ? (
                    <img src={preview} alt="Preview" className="mt-4 max-h-48 w-full max-w-md rounded-2xl object-contain" />
                  ) : null}
                </label>
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Button onClick={handleAnalyze} disabled={!file || isAnalyzing}>
                  {isAnalyzing ? <span className="flex items-center gap-2"><Spinner className="size-4" /> Analyzing...</span> : 'Analyze image'}
                </Button>
              </div>
                ...
                <div className="grid gap-4 rounded-3xl border border-border bg-background p-5">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-2xl border border-border bg-muted p-4">
                      ...
                      <p className="mt-2 break-all text-sm font-medium">{analysis.hash}</p>
                    </div>
                    <div className="rounded-2xl border border-border bg-muted p-4">
                      <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Model</p>
                      <p className="mt-2 text-sm font-medium">{analysis.model_id}</p>
                    </div>
                  </div>
                </div>
              ) : null}
            ...
            {analysis ? (
              <CardFooter className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Button onClick={handleCommit} disabled={isCommitting}>
                  {isCommitting ? <span className="flex items-center gap-2"><Spinner className="size-4" /> Committing...</span> : 'Commit to Midnight'}
                </Button>
              </CardFooter>
            ) : null}
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Privacy is preserved by keeping raw images and model analysis off-chain.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4">
              <div className="rounded-3xl border border-border bg-muted p-5">
                <p className="text-sm font-semibold">1. Analyze privately</p>
                <p className="mt-2 text-sm text-muted-foreground">The backend runs deepfake detection and hashes the image.</p>
              </div>
              <div className="rounded-3xl border border-border bg-muted p-5">
                <p className="text-sm font-semibold">2. Generate proof</p>
                <p className="mt-2 text-sm text-muted-foreground">The contract records only the fingerprint and verdict, not the raw image.</p>
              </div>
              <div className="rounded-3xl border border-border bg-muted p-5">
                <p className="text-sm font-semibold">3. Verify anytime</p>
                <p className="mt-2 text-sm text-muted-foreground">Anyone can lookup a hash on the verification page to confirm the published verdict.</p>
              </div>
            </CardContent>
            ...
            {commitResult ? (
              <CardFooter className="flex flex-col gap-3">
                <div className="rounded-3xl border border-border bg-background p-4">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">TX ID</p>
                  <p className="mt-1 break-all text-sm font-medium">{commitResult.tx_id}</p>
                </div>
              </CardFooter>
            ) : null}
          </Card>
